function assemblyTuning_selectedPeriods_rem(sessionNumber)
...
for binDur = [0.25 0.5]
    ...
    for iEpoch = [1 3]
        ...
        if ~isempty(currEpoch_includeIdx)
...
%                     concatBoutIDs.(currEpoch), boutFirings_unit.(currEpoch).data] = calculateAssemblyTuning_selectedPeriods(currBouts, spikes, binDur, clusterQuality, ifShuffle);
            ...
            ifShuffle = 1;
            assemblyTunings.(currEpoch).ui = calculateAssemblyTuning_selectedPeriods(currBouts, spikes, binDur, clusterQuality, ifShuffle);
            ...
            for idata = 1:2
                ...
                allCorrs = corr(assemblyTunings.(currEpoch).(currDataType)', spatialTunings');
                assemblyTuningPFcorr.(currEpoch).(currDataType) = diag(allCorrs);

                allKLdiv = calKLDivergence(assemblyTunings.(currEpoch).(currDataType)', spatialTunings');
                assemblyTuningPFKLdiv.(currEpoch).(currDataType) = diag(allKLdiv);

                [assemblyTuningPF_prefPosDist.(currEpoch).(currDataType), assemblyTuning_prefPos.(currEpoch).(currDataType)] = ...
                    calDistPrefPosition(assemblyTunings.(currEpoch).(currDataType), peakLoc, []);
    ...
    fileName = sprintf('%s.assemblyTunings_REM_%.3f.mat', sessionName, binDur);
