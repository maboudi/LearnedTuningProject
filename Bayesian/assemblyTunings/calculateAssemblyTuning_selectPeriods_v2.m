function [assemblyTunings, concatBinCenters, concatBoutIDs, boutFirings_unit] = calculateAssemblyTuning_selectPeriods_v2(bouts, spikes, clusterQuality, ifshuffle)

% Compared to the previous version, here I concatenated all of the bouts
% and calculated a single learned tuning for each unit
...
for ib = 1:nBouts
    binnedFirings{ib} = timeBinning_20(bouts(ib, :), spikes, binDur, 0, fileInfo);
    
    nBins = size(binnedFirings{ib}, 2);
    
    binCenters{ib}    = bouts(ib, 1) + (1:nBins)*binDur - binDur/2; 
...
if ifshuffle == 1 
   ...
   % % generate a unit ID shuffle surrogate
    ...
    concatBinnedFirings_ui = zeros(size(concatBinnedFirings));
...
%     concatPBEfirings_ui(lowFiringUnits, :)  = concatPBEfirings(lowFiringUnits(randperm(numel(lowFiringUnits))), :);  
    ...
    concatBinnedFirings_ui(activeUnits, :) = concatBinnedFirings(activeUnits(randperm(nActiveUnits)) , :);
    concatBinnedFirings = concatBinnedFirings_ui;
...
%     for ib = 1:nBouts
%         binnedFirings_ui{ib}(highFiringUnits, :) = binnedFirings{ib}(highFiringUnits(randperm(numel(highFiringUnits))), :);
%         binnedFirings_ui{ib}(lowFiringUnits, :)  = binnedFirings{ib}(lowFiringUnits(randperm(numel(lowFiringUnits))), :);
%     end
...
%     concatBinnedFirings_OtherUnits = concatBinnedFirings(otherUnits, :); 
    ...
    idx = sum(posteriorProbMatrix_allBouts, 1) > 0 & boutFirings_unit(currUnit, :) > 0 & sum(boutFirings_otherUnits, 1) > 0;
    ...
    if ~all(~idx)
        ...
        indivBinTuning = (posteriorProbMatrix_allBouts * numel(find(boutFirings_unit(currUnit, :) > 0))/numel(boutFirings_unit(currUnit, :) > 0))./repmat(px, [1, size(posteriorProbMatrix_allBouts, 2)]);
