function assemblyTuning_selectedPeriods_moreShuffles_REM(sessionNumber)
...
%         MakeEvtFile(currBouts, fullfile(fullfile(basePath, [sessionName '.the.evt'])), {'beg', 'end'}, 1);
...
%         assemblyTuningPF_prefPosDist_pscore.(currEpoch) = calPrctile(assemblyTuningPF_prefPosDist.(currEpoch));
    ...
    fileName = sprintf('%s.assemblyTunings_REMcontamineatedWithPBEs_%.3f.mat', sessionName, binDur);
