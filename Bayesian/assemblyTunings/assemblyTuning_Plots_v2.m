...
% load(fullfile(basePath, 'BayesianDecoding', [sessionName '.PBEInfo_replayScores.mat']))
...
subplot(5,3,5)
placeFieldCorrs = corr(sortedPFs', sortedPFs');
imagesc(placeFieldCorrs)
...
% % correlation matrix consisted of correlation values between assembly
% tunings and place fields of all units
...
allCorrValues = [assemblyTuningPFcorr.pre.data; assemblyTuningPFcorr.run.data; assemblyTuningPFcorr.post.data];
...
for iperiod = 1:3
    ...
    h = histc(assemblyTuningPFcorr.(currPeriod).data, bins); h(end) = [];
    a(iperiod ) = bar(bins(1:end-1)+diff(bins(1:2))/2, h/nUnits, 'Edgecolor', edgeColors{iperiod}, 'facecolor', colors{iperiod}, 'facealpha', 0.3);
    hold on
    
    rr = ylim;
    medianCorr = nanmedian(assemblyTuningPFcorr.(currPeriod).data);
...
allSpatialInfo = [assemblyTuningSpatialInfo.pre.data; assemblyTuningSpatialInfo.run.data; assemblyTuningSpatialInfo.post.data]; 
...
for iperiod = 1:3
    ...
    h = histc(assemblyTuningSpatialInfo.(currPeriod).data, bins); h(end) = [];
    a(iperiod ) = bar(bins(1:end-1)+diff(bins(1:2))/2, h/nUnits, 'Edgecolor', edgeColors{iperiod}, 'facecolor', colors{iperiod}, 'facealpha', 0.3);

    hold on
    
    rr = ylim;
    medianCorr = nanmedian(assemblyTuningSpatialInfo.(currPeriod).data);
    cl = colors{iperiod}; 
    if iperiod == 2; cl = 'k'; end
    line([medianCorr medianCorr], [0 rr(2)], 'color', cl, 'linewidth', 2)
...
subplot(17,3, 45)
scatter(assemblyTuningSpatialInfo.run.data, assemblyTuningPFcorr.run.data, 10, 'MarkerFaceColor', 'k', 'MarkerFaceAlpha',.5, 'MarkerEdgeColor','none')
xlim([0 prctile(assemblyTuningSpatialInfo.run.data, 90)])
legend('RUN')

subplot(17,3, [48 51])
scatter(assemblyTuningSpatialInfo.pre.data, assemblyTuningPFcorr.pre.data, 10, 'MarkerFaceColor', 'b', 'MarkerFaceAlpha',.5, 'MarkerEdgeColor','none')
hold on
scatter(assemblyTuningSpatialInfo.post.data, assemblyTuningPFcorr.post.data, 10, 'MarkerFaceColor', 'r', 'MarkerFaceAlpha',.5, 'MarkerEdgeColor','none')


xlim([0 prctile([assemblyTuningSpatialInfo.pre.data; assemblyTuningSpatialInfo.post.data], 90)])
xlabel('spatial information', 'fontsize', 12)
...
replayScoreMethod      = replayScoreMethods{methodNumber};
replayScoreMethod_name = replayScoreMethods_fullName{methodNumber};
...
for iperiod = 1:3
    ...
    h = violinplot(asTuningSpatialInfo_sub.(currPeriod).data.(replayScoreMethod));
    ...
    dim = [0.1 0.8 0.5 .1];
    annotation('textbox',dim,'String', {sessionName; replayScoreMethod_name}, ...
...
for iperiod = 1:3
    ...
    h = violinplot(asTuningPFcorr_sub.(currPeriod).data.(replayScoreMethod));
...
%                 validUnits = ~isnan(asTuningPFcorr_sub.(currPeriod)(:, ii));
        dataMedian    = median(asTuningPFcorr_sub.(currPeriod).data.(replayScoreMethod)(:, ii));
        shuffleMedian = median(corrWithPFs_ui.(currPeriod).data.(replayScoreMethod)(:, ii));
        line([ii-0.3 ii+0.3], [shuffleMedian shuffleMedian], 'color', colors{iperiod}, 'linestyle', ':', 'linewidth', 2)

        pval = ranksum(asTuningPFcorr_sub.(currPeriod)(:, ii), corrWithPFs_ui.(currPeriod)(:, ii), 'tail', 'right');
...
for ii = 1:2
    ...
    for jj = 1:2
        ...
        currMat = normAssemblyTunings_subset.(currShuffleMethod).(currRepScorMethod)(sortInd(1:nUnits), :);
        imagesc(currMat)
        xlabel('position bin')
