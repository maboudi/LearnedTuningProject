...
%         allKLdivs = calKLDivergence(currTimeBinLTs', currPFs');
        ...
        shuffle_KLdiv  = nan(nUnits, nShuffles);
        
        for i_s = 1:nShuffles    
            cidx = randi(nUnits, nUnits, 1);
            shuffle_PFcorr(:, i_s) = allCorrs(sub2ind(size(allCorrs), (1:nUnits)', cidx));
%             shuffle_KLdiv(:, i_s)  = allKLdivs(sub2ind(size(allKLdivs), (1:nUnits)', cidx));
        end

        median_shufflePFcorrs = prctile(shuffle_PFcorr, 50);
%         median_shuffleKLdivs  = median(prctile(shuffle_KLdiv, 50));

        PFcorrs_prctileScore(:, iTimeBin) = (diag(allCorrs) - mean(median_shufflePFcorrs)); %/std(median_shufflePFcorrs);
%         KLdiv_prctileScore(:, iTimeBin)   = diag(allKLdivs) - median_shuffleKLdivs;
