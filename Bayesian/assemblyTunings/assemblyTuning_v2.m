function assemblyTuning_v2(sessionNumber)
...
load(fullfile(basePath, 'PopulationBurstEvents', [sessionName '.PBEInfo_all.mat']), 'PBEInfo')
...
for iEpoch = 1:numel(epochNames) 
    ...
    switch iEpoch
        ...
        case 3 % POST
%             idx  = peaks > (epochs(3,1)+6*60*60) & peaks < epochs(3,2);
%             idx  = peaks > (epochs(3,2)-4*60*60) & peaks < epochs(3,2);
            idx  = peaks > epochs(3,1)+2*60*60 & peaks < epochs(3,1)+4*60*60;
...
fileName = [sessionName '.assemblyTunings_allPBEs_2nd_2hoursPOST.mat']; % after6hoursPost
...
for irsm = [1 2 5 6 7 8]
    ...
    for iEpoch = 1:3 % behavioral epochs: pre, run, post
        ...
        for iPBEset = 1:4
            ...
            ifShuffle = 0;
            currAssemblyTunings = calculateAssemblyTuningV8(currbinnedPBEs, spikes, selectPBEs, clusterQuality, ifShuffle);
            ...
            asTuningPFcorr_sub.(currEpoch).(replayScoreMethod).data(:, iPBEset)     = diag(asTuningCorrMat_sub.(currEpoch).data.(replayScoreMethod)(:, :, iPBEset));
