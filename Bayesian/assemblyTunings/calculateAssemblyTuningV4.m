function [assemblyTunings, px, spikeCountProbs, nCoactiveUnits_wEachUnit, nCoactiveUnits] = calculateAssemblyTuningV4(PBEbinnedFirings, placeFieldsRL, placeFieldsLR, selectPBEs, shankIDs, ifshuffle)

% compared to version 3, we added the input unit ID. In calculation of
% assemblyTuning for a given unit, we consider firings of only units that are on different shanks.   
...
% Also, time bins are included during which at least one other unit fired. 
...
for iUnit = 1:nActiveUnits
    ...
    % % added Jan 13 2021
    otherUnits = find(shankIDs ~= shankIDs(currUnit));
    % %
...
%     nonSilentBinsIdx = find(concatPBEfirings_unit > 0 & sum(concatPBEfirings_OtherUnits, 1) > 0);
%     
