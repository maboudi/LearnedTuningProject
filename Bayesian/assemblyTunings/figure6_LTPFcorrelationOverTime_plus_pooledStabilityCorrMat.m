...
nPOST_Blocks = numel(h)-1;
p_value = nan(nPOST_Blocks);

for ii = 2:nPOST_Blocks
    for jj = ii-1
        try 
            [p_value(ii, jj), ~, stats] = signrank(POST_longStability_pooled(:,ii), POST_longStability_pooled(:,jj));
        catch
        end
    end
end
