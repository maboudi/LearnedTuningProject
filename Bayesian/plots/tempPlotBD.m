function tempPlotBD(eventsBinnedFiring, posteriorProbMatrix, BDseqscore, weightedCorr, begPosition, endPosition, L2R_template, R2L_template, PBEidx2plot, binDur, fileBase, textOnTop)
...
%     end
    ...
    for i = 1:nor
        ...
        for j = 1:noc
            ...
            for unit = 1 : size(spikesR2L,1)
                
                spikes = find(spikesR2L(unit,:));
                
                if ~isempty(spikes)
                    for spk = 1 : length(spikes)
                        plot([spikes(spk), spikes(spk)], [unit-1.25, unit+1.25], 'color', 'k', 'linewidth', 1) % colorset(ceil(L2R_template(unit)*128/noUnits), :)
                        hold on
                    end
                end
            ...
            xlim([0 size(spikesR2L, 2)])
            ylim([0 length(L2R_template)+3])
%             title(sprintf('R2L %.1f\nL2R %.1f', BDseqscore(event, 1) , BDseqscore(event, 2)),'fontsize', 8, 'Units', 'normalized', 'Position', [0 1], 'HorizontalAlignment', 'left')
            
            if mod(eventInd, noc) == 1
               ylabel('Unit', 'fontsize', 6);
               set(gca, 'xtick',[], 'ytick',[1 size(spikesR2L,1)], 'yticklabel', {'1',sprintf('%d', size(spikesR2L,1))})
               title({['pbe ' num2str(event)], 'Template 1'},'fontsize', 6, 'Units', 'normalized', 'Position', [0 1], 'HorizontalAlignment', 'left')
            ...
            for unit = 1:size(spikesL2R,1)
                
                spikes = find(spikesL2R(unit,:));
                if ~isempty(spikes)
                    for spk = 1 : length(spikes)
                        plot([spikes(spk) spikes(spk)], [unit-1.25 unit+1.25], 'color', 'k','linewidth', 1) %%, colorset(ceil(R2L_template(unit)*128/noUnits), :)
                        hold on
                    end
                end
                
            end
            xlim([0 size(spikesL2R, 2)])
            ylim([0 length(R2L_template)+3])
            
            
            if mod(eventInd, noc) == 1
               ylabel('Unit', 'fontsize', 6);
               set(gca, 'xtick',[], 'ytick',[1 size(spikesL2R,1)], 'yticklabel', {'1',sprintf('%d', size(spikesL2R,1))})
               title('Template 2','fontsize', 6, 'Units', 'normalized', 'Position', [0 1], 'HorizontalAlignment', 'left')
...
%             ax1 = axes('position',sub_pos2{1,1},'XGrid','off','XMinorGrid','off','FontSize',fontsize,'Box','on','Layer','top');
            ...
            plot([begPosition(event,1,1) endPosition(event,1,1)], [begPosition(event,2,1)-7 endPosition(event,2,1)-7],'color','w', 'linestyle', ':', 'linewidth', 0.5)
            plot([begPosition(event,1,1) endPosition(event,1,1)], [begPosition(event,2,1)+7 endPosition(event,2,1)+7],'color','w', 'linestyle', ':', 'linewidth', 0.5)
