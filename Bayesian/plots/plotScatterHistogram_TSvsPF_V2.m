function plotScatterHistogram_TSvsPF_V2(BDscores, directory)
...
for jj = 1: length(BDReplayMetrics)
    ...
    for ii = 1: length(periods)
        ...
        xdata = BDscores.(period).wPBEtimeswap.(BDReplayMetric);
        ydata = BDscores.(period).unitIDshuffle.(BDReplayMetric);
    ...
    if strcmp(BDReplayMetric, 'replayScore')
       annot = 'Radon Transform';
    elseif strcmp(BDReplayMetric, 'weightedCorr')
       annot = 'Weighted Correlation';
    end
        
    annotation('textbox',dim,'String', annot, 'FitBoxToText','on', 'Interpreter', 'none');

    
    filename = fullfile(directory, ['TimeSwap_vs_unitIDshuffle_' annot]);
