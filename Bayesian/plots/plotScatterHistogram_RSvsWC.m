function plotScatterHistogram_RSvsWC(BDscores, acceptedEvts, fileinfo, directory)
...
for jj = 1:length(shuffleMethods)
    ...
    period = periods{ii};    
    shuffleMethod = shuffleMethods{jj};
    
    xdata = BDscores.(period).data.(shuffleMethod).weightedCorr.prctilescore(acceptedEvts.(period), 1);
    ydata = BDscores.(period).data.(shuffleMethod).replayScore.prctilescore(acceptedEvts.(period), 1);
    ...
    xAxis  = 'weighted correlation percentile';
    yAxis  = 'Radon transform percentile';
    ...
    dim = [0.15 0.6 0.5 .1];
    annotation('textbox',dim,'String',{fileinfo.name; shuffleMethod}, 'FitBoxToText','on', 'Interpreter', 'none');

    
    filename = fullfile(directory, ['wdcVSwc_' shuffleMethod]);
