function [posteriorProbMatrix, replayScore, weightedCorr, jumpDist, BDseqscore, sigMatrix, onlyLineElements, begPosition, endPosition, replayScore_null, weightedCorr_null] = BDreplayDetect_polar_1D(eventsBinnedfiring, tuning, activeUnits, binDur)
...
%         [replayScore_nullLR_pbe(sn), weightedCorr_nullLR_pbe(sn), jumpDist_nullLR_pbe(sn, :)] = replayevaluation_polar(posteriorProbMatrixLR_shuffle);
    ...
    for dir = 1%1:2
        ...
        BDseqscore.weightedCorr.zscore(pbe, dir)       = (abs(weightedCorr(pbe, dir)) - mean(abs(weightedCorr_null(pbe, dir, :)))) / std(abs(weightedCorr_null(pbe, dir, :)));
