function [posteriorProbMatrix, RadonTF, weightedCorr, jumpDist, BDseqscore, sigMatrix, onFitLineElements, begPosition, endPosition, RadonTF_null, weightedCorr_null, coveredLen, coveredLen_null, jumpDist_null] = BDreplayDetect_vMarch21(eventsBinnedfiring, tuningRL, tuningLR, activeUnits, binDur)
...
spatialTuningRL = tuningRL.smoothed; 
unsmoothedSpatialTuningRL = tuningRL.unsmoothed;

spatialTuningLR = tuningLR.smoothed; 
unsmoothedSpatialTuningLR = tuningLR.unsmoothed;
...
if isempty(activeUnits)
   activeUnits = 1:size(spatialTuningRL, 1); 
end
...
% fitLineDictionary = generateLineDictionary(nPosBins, 1:30);
...
for pbe = 1:nEvents
    ...
    postprobRL = baysDecoder(currEvent, spatialTuningRL(activeUnits, :), binDur); 
    postprobLR = baysDecoder(currEvent, spatialTuningLR(activeUnits, :), binDur);

 
    %%% Summed probability distribution, every column is normalized to one
    %%% (this is the same method as used in Davidson et al. 2009)
    
    posteriorProbMatrix{pbe} = (postprobRL + postprobLR)./repmat(sum(postprobRL + postprobLR, 1), [nPosBins, 1]);
    ...
    [RadonTF(pbe), weightedCorr(pbe), pbeJumpDist, onFitLineElements{pbe}, begPosition(pbe,:), endPosition(pbe,:)] = replayevaluation_polar2(posteriorProbMatrix{pbe, 1}, fitLineDictionary);
...
%     posteriorProbMatrixLR = posteriorProbMatrix(pbe, 2);
    ...
    BDseqscore.wPBEtimeswap.RadonTF.zscore(pbe)        = (RadonTF(pbe) - mean(RadonTF_null(pbe, :, 1)))/std(RadonTF_null(pbe, :, 1));
...
%     temp = zeros(m,n,50);
    ...
    BDseqscore.unitIDshuffle.RadonTF.zscore(pbe)        = (RadonTF(pbe) - mean(RadonTF_null(pbe, :, 2)))/std(RadonTF_null(pbe, :, 2));
...
%            pbe, max(BDseqscore.RadonTF.prctilescore(pbe, :)), max(BDseqscore.weightedCorr.prctilescore(pbe, :)), max(BDseqscore.RadonTF.zscore(pbe, :)), max(BDseqscore.weightedCorr.zscore(pbe, :)))
       fprintf('\n event #%d - %d: pscore rs %.1f wc %.1f  and zscore rs %.2f wc %.2f', ...
...
for pbe = 1:nEvents
    ...
    RadonTF_null(pbe, :, 3)      = permute(RadonTF_null_pbe, [2, 1]); % concatenating two directions and permuting the dimensions 
    ...
    BDseqscore.circularPFshuffle.RadonTF.zscore(pbe)        = (RadonTF(pbe) - mean(RadonTF_null(pbe, :, 3)))/std(RadonTF_null(pbe, :, 3));
    ...
    BDseqscore.circularPFshuffle.weightedCorr.zscore(pbe)       = (abs(weightedCorr(pbe)) - mean(abs(weightedCorr_null(pbe, :, 3)))) / std(abs(weightedCorr_null(pbe, :, 3)));
...
for pbe = 1:nEvents
    ...
    BDseqscore.circularPosteriorShuffle.RadonTF.zscore(pbe)        = (RadonTF(pbe) - mean(RadonTF_null(pbe, :, 4)))/std(RadonTF_null(pbe, :, 4));
    ...
    BDseqscore.circularPosteriorShuffle.weightedCorr.zscore(pbe)       = (abs(weightedCorr(pbe)) - mean(abs(weightedCorr_null(pbe, :, 4)))) / std(abs(weightedCorr_null(pbe, :, 4)));
