function [posteriorProbMatrix, replayScore, weightedCorr, jumpDist, BDseqscore, sigMatrix, onFitLineElements, begPosition, endPosition, replayScore_null, weightedCorr_null, coveredLen, coveredLen_null, jumpDist_null] = BDreplayDetect_vFeb20(eventsBinnedfiring, tuningRL, tuningLR, activeUnits, binDur)
...
%     posteriorProbMatrixLR = posteriorProbMatrix(pbe, 2);
    ...
    BDseqscore.wPBEtimeswap.replayScore.zscore(pbe)        = (replayScore(pbe) - mean(replayScore_null(pbe, :, 1)))/std(replayScore_null(pbe, :, 1));
    ...
    if mod(pbe, 50) == 0
       fprintf('\n event #%d - %d: pscore rs %.1f wc %.1f  and zscore rs %.2f wc %.2f', ...
           pbe-49, pbe, mean(BDseqscore.wPBEtimeswap.replayScore.prctilescore(pbe-49:pbe, 1)), mean(BDseqscore.wPBEtimeswap.weightedCorr.prctilescore(pbe-49:pbe, 1)), mean(BDseqscore.wPBEtimeswap.replayScore.zscore(pbe-49:pbe, 1)), mean(BDseqscore.wPBEtimeswap.weightedCorr.zscore(pbe-49:pbe, 1)))
    end
...
%     temp = zeros(m,n,50);
    ...
    BDseqscore.unitIDshuffle.replayScore.zscore(pbe)        = (replayScore(pbe) - mean(replayScore_null(pbe, :, 2)))/std(replayScore_null(pbe, :, 2));
...
%            pbe, max(BDseqscore.replayScore.prctilescore(pbe, :)), max(BDseqscore.weightedCorr.prctilescore(pbe, :)), max(BDseqscore.replayScore.zscore(pbe, :)), max(BDseqscore.weightedCorr.zscore(pbe, :)))
       fprintf('\n event #%d - %d: pscore rs %.1f wc %.1f  and zscore rs %.2f wc %.2f', ...
           pbe-49, pbe, mean(BDseqscore.unitIDshuffle.replayScore.prctilescore(pbe-49:pbe, 1)), mean(BDseqscore.unitIDshuffle.weightedCorr.prctilescore(pbe-49:pbe, 1)), mean(BDseqscore.unitIDshuffle.replayScore.zscore(pbe-49:pbe, 1)), mean(BDseqscore.unitIDshuffle.weightedCorr.zscore(pbe-49:pbe, 1)))
    end
