function [PBEInfo, PBEInfo_null, sigMatrix] = BDreplayDetect_vJune21(PBEInfo, spikes, fileInfo, binDur)
...
spatialTuningRL = zeros(nUnits, 1);
unsmoothedSpatialTuningRL = zeros(nUnits, 1);

spatialTuningLR = zeros(nUnits, 1);
unsmoothedSpatialTuningLR = zeros(nUnits, 1);
...
for pbe = 1:nPBEs
    ...
    postprobRL = baysDecoder(currPBE, spatialTuningRL(activeUnits, :), binDur); 
    postprobLR = baysDecoder(currPBE, spatialTuningLR(activeUnits, :), binDur);

 
    %%% Summed probability distribution, every column is normalized to one
    %%% (this is the same method as used in Davidson et al. 2009)
    
    posteriorProbMat = (postprobRL + postprobLR)./repmat(sum(postprobRL + postprobLR, 1), [nPosBins, 1]);
