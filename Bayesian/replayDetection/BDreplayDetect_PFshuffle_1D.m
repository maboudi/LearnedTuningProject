function [posteriorProbMatrix, replayScore, weightedCorr, BDseqscore] = BDreplayDetect_PFshuffle_1D(eventsBinnedfiring, tuning, tuningshuffles, activeUnits, binDur)
...
%     [replayScore(pbe, 3), slope(pbe, 3), rho(pbe, 3), begPosition(pbe,:,3), endPosition(pbe,:,3)] = replayevaluation(posteriorProbMatrix{pbe, 3});
    ...
    [replayScore(pbe, 1), weightedCorr(pbe, 1)] = replayevaluation_polar2(posteriorProbMatrix{pbe, 1}, lineDictionary);
...
numPFshuffles = size(tuningshuffles, 3);
...
for pbe = 1: nEvents
    ...
    parfor ss = 1:numPFshuffles % shuffle index
        ...
        postprob = baysDecoder(currEvent, tuningshuffles(activeUnits, :, ss), binDur); 

        posteriorProbMatrix_PFshuffle = postprob./repmat(sum(postprob, 1), [nPositionBins, 1]);
        posteriorProbMatrix_PFshuffle(:, idx) = 0;

        lineDictionary = generateLineDictionary(nPositionBins, nTimebins);
        [replayScore_PFshuffle_pbe(ss), weightedCorr_PFshuffle_pbe(ss)] = replayevaluation_polar2(posteriorProbMatrix_PFshuffle, lineDictionary);
