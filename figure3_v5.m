...
% a bit cleaner than the previous one
...
load(fullfile(basePath, 'assemblyTunings', [sessionName '.assemblyTunings_allPBEs.mat']), 'assemblyTunings');


% recalculating the PF corrrelation for data and shuffe 
% (this shuffle is different from randomizing the co-activity during  the PBEs. In this one we consider the pearson correlations (or KL divergence) between non-matching learned tunings and place fields as the null dsitribution)
...
for iEpoch = 1:3
    ...
    % data
    assemblyTuningPFcorr{iSess}.(currEpoch).data  = diag(corrMat);
    assemblyTuningPFKLdiv{iSess}.(currEpoch).data = diag(KLdivergence);
    
    
    % shuffle: basically every non-disgonal element of the correlation
    % matrix
%     
...
%     end
%     nOffDiagonals = nUnits*(nUnits - 1);
%     
%     
%     
%     % PF correlation
%     offDiagonals = corrMat(offDiagonalIdx);
%     
%     assemblyTuningPFcorr{iSess}.(currEpoch).ui     = offDiagonals(randperm(nOffDiagonals, min(nOffDiagonals, nShuffles)));
%     assemblyTuningPFcorr_zscore{iSess}.(currEpoch) = (assemblyTuningPFcorr{iSess}.(currEpoch).data - nanmean(assemblyTuningPFcorr{iSess}.(currEpoch).ui)) ./ nanstd(assemblyTuningPFcorr{iSess}.(currEpoch).ui);
   
    for ii = 1:nUnits; corrMat(ii, ii) = nan; KLdivergence(ii, ii) = nan; end
    assemblyTuningPFcorr{iSess}.(currEpoch).ui     = corrMat;
    assemblyTuningPFcorr_zscore{iSess}.(currEpoch) = (assemblyTuningPFcorr{iSess}.(currEpoch).data - nanmean(assemblyTuningPFcorr{iSess}.(currEpoch).ui, 1)') ./ nanstd(assemblyTuningPFcorr{iSess}.(currEpoch).ui, [], 1);

    
    % KL divergence
%     offDiagonals = KLdivergence(offDiagonalIdx);
%     
%     assemblyTuningPFKLdiv{iSess}.(currEpoch).ui     = offDiagonals(randperm(nOffDiagonals, min(nOffDiagonals, nShuffles))); 
%     assemblyTuningPFKLdiv_zscore{iSess}.(currEpoch) = (assemblyTuningPFKLdiv{iSess}.(currEpoch).data - nanmean(assemblyTuningPFKLdiv{iSess}.(currEpoch).ui)) ./ nanstd(assemblyTuningPFKLdiv{iSess}.(currEpoch).ui);    

    assemblyTuningPFKLdiv{iSess}.(currEpoch).ui     = KLdivergence;
    assemblyTuningPFKLdiv_zscore{iSess}.(currEpoch) = (assemblyTuningPFKLdiv{iSess}.(currEpoch).data - nanmean(assemblyTuningPFKLdiv{iSess}.(currEpoch).ui, 1)') ./ nanstd(assemblyTuningPFKLdiv{iSess}.(currEpoch).ui, [], 1);
...
%% calculate the PF correlations for data and shuffle for the 15-minutes moving time windows
 

offDiagonalIdx = true(nUnits);
for ii = 1:nUnits
    offDiagonalIdx(ii, ii) = false;
end
...
for iEpoch = 1:3
    ...
    allLTs = assemblyTunings_time_nonOverlap{iSess}.(currEpoch).data;

    nTimeBins = size(allLTs, 3);

    assemblyTuningPFcorr_time{iSess}.(currEpoch).data = nan(nUnits, nTimeBins);
    assemblyTuningPFcorr_time{iSess}.(currEpoch).ui   = nan(nShuffles, nTimeBins);
    
    assemblyTuningPFKLdiv_time{iSess}.(currEpoch).data  = nan(nUnits, nTimeBins);
    assemblyTuningPFKLdiv_time{iSess}.(currEpoch).ui    = nan(nShuffles, nTimeBins);
    
    
    for iTimeBin = 1:nTimeBins
        
        currLT = allLTs(:, :, iTimeBin);
        
        corrMat = corr(currLT', spatialTunings_merge', 'type', 'pearson');
        KLdivergence = calKLDivergence(currLT', spatialTunings_merge');
        
        assemblyTuningPFcorr_time{iSess}.(currEpoch).data(:, iTimeBin) = diag(corrMat);
        assemblyTuningPFKLdiv_time{iSess}.(currEpoch).data(:, iTimeBin)  = diag(KLdivergence);
        

        % PF correlation
        offDiagonals = corrMat(offDiagonalIdx);
        offDiagonals(isnan(offDiagonals)) = [];
        nOffDiagonals = numel(offDiagonals);

        if ~isempty(offDiagonals)
            nShuffles2 = min(nOffDiagonals, nShuffles);
            assemblyTuningPFcorr_time{iSess}.(currEpoch).ui(1:nShuffles2, iTimeBin) = offDiagonals(randperm(nOffDiagonals, nShuffles2));
        end
        ...
        if ~isempty(offDiagonals)
            nShuffles2 = min(nOffDiagonals, nShuffles);
            assemblyTuningPFKLdiv_time{iSess}.(currEpoch).ui(1:nShuffles2, iTimeBin)  = offDiagonals(randperm(nOffDiagonals, nShuffles2)); 
        end
        
    end

    
    assemblyTuningPFcorr_time_zscore{iSess}.(currEpoch) = ((assemblyTuningPFcorr_time{iSess}.(currEpoch).data) - ...
        nanmean(assemblyTuningPFcorr_time{iSess}.(currEpoch).ui(:))) ./ nanstd(assemblyTuningPFcorr_time{iSess}.(currEpoch).ui(:));
    
    assemblyTuningPFKLdiv_time_zscore{iSess}.(currEpoch) = ((assemblyTuningPFKLdiv_time{iSess}.(currEpoch).data) - ...
        nanmean(assemblyTuningPFKLdiv_time{iSess}.(currEpoch).ui(:))) ./ nanstd(assemblyTuningPFKLdiv_time{iSess}.(currEpoch).ui(:));
end
...
% ylabel('PF-LT Pearson correlation coeff.', 'fontsize', fontsize)
...
idx = KLdivs_all > prctile(KLdivs_all, 1) & KLdivs_all < prctile(KLdivs_all, 99) & PFcorrs_zscore_all > prctile(PFcorrs_zscore_all, 0) & PFcorrs_zscore_all < prctile(PFcorrs_zscore_all, 100);
