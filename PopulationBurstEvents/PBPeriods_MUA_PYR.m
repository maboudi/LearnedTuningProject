function [PBEs, sdat] = PBPeriods_MUA_PYR(MUA, PYR, detectParams, fileInfo)
...
highRateBins = find(sdat.MUA > threshZ); 
...
for ii = 1 : nPrimPeriods
    ...
    [primpeak(ii), Ind] = max(sdat.MUA(primPeriodStr(ii) : primPeriodEnd(ii))); %% Ind signifiy the center bin in respect to the current period
...
%     if is == 2 % removing extra long (or short) detected events from the detected PBEs- only PYR
