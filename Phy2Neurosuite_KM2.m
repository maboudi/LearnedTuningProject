function Phy2Neurosuite_KM2(sh)

% this function extract the wavforms and computes PCs, if we already have
% the spike times, .xml and .dat files.


addpath(genpath('/home/kouroshmaboudi/Documents/NCMLproject/ReplayPreplayAnalyses'))

% fileBase = '/nfs/turbo/umms-kdiba/Kourosh/NCMLproject/RatU_Day2/RatU_Day2NSD_2021-07-24_08-16-38';
fileBase = '/data/datasets/Bapun_Datasets_RatN&S_normalSleep/RatS_Day2/RatS-Day2NSD-2020-11-27_10-22-29';
...
wpowers = [wpowers int64(sum(waveforms2(:, ii*segLen+1:end).^2,1)/size(waveforms2(:, ii*segLen+1:end),1)/100)]; % he remaining
...
for k = 1:size(waveforms,1) % for each channel
    PCAs_global(:,k,:) = pca(zscore(permute(waveforms(k,:,:),[2,3,1]),[],2),'NumComponents',3)'; % do we need the z-scoring here before PCA??
end
