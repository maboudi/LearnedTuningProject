function [Out1, Out2, Out3, Out4] = fastica(mixedsig, varargin)
...
if jumpWhitening == 3
  ...
  if jumpPCA == 2,
    ...
    % Calculate PCA
    [E, D, allEigValues]=pcamat(mixedsig, firstEig, lastEig, interactivePCA, verbose);
    
    
    
    %% added by KM(May 2020)
    % considering only PCs with eigen values above eigValueThresh
    
    if exist('eigValueThresh', 'var')
        
        signPCs = find(diag(D) >= eigValueThresh);
