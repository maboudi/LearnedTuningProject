...
binOverlapRatio = struct('coarseTS', 0.9, 'fineTS', 0);
...
for its = 1:2
    ...
    [ICs.(currTimeScale), ICweights.(currTimeScale), eigenValues.(currTimeScale), evThresh.(currTimeScale)] = pca_ica_v3(runTraining.(currTimeScale), 'firingmean', meanFiringCount, 'firingstd', stdFiringCount);
...
for ic = 1: size(ICweights.coarseTS, 1)
...
%         ICtunings.(currTimeScale)(:, ii) = sum(activationStrength.(currTimeScale).RUN.original(:, posbinIdx.(currTimeScale)(:, 1) == uniqPositions(ii)), 2); 
        ICtunings.(currTimeScale)(:, ii) = sum(activationStrength.(currTimeScale).RUN.original(:, posbinIdx.(currTimeScale)(:, 1) == uniqPositions(ii)), 2)/numel(find(posbinIdx.(currTimeScale)(:, 1) == uniqPositions(ii))); 
    end
...
for iperiod = 1:3
    ...
    currPeriod = periodNames{iperiod};
    ax0.(currPeriod) = subplot(10,3, ((1:6)-1)*3 +iperiod);    
    ...
    plot(currBinCenters-startTimes(iperiod), currActivationStrs(:, startBins(iperiod)+(0:noBins2Plot-1))', 'linewidth',1.5)
    hold on
...
%        line([0 0], [400 500], 'linewidth', 2, 'color', 'k')
    ...
    % ripple LPF 
    ax1.(currPeriod) = subplot(10,3, (7-1)*3 +iperiod); 
    ...
    % population burst
    ax2.(currPeriod) = subplot(10,3, (8-1)*3 +iperiod); 
    ...
    % BD replay scores
    ax3.(currPeriod) = subplot(10, 3, ((9:10)-1)*3 + iperiod);
