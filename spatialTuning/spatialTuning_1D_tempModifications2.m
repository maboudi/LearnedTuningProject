function [spatialTunings, PF_sorted, template, spatialInfo, conslapsRatio, diffWithAvg] = spatialTuning_1D_tempModifications2(spikeStruct, qclus, fileinfo, behavior, thetaPeriods, turningPeriods, speed, direction, posBinSize, runSpeedThresh, combinedUnits, timeUnit, FileBase)
...
conslapsRatio = zeros(totoalNumofUnits, 1);
spatialInfo   = zeros(totoalNumofUnits, 1);
...
for ii = 1: length(activeUnits)
    ...
    [~, peakPosBins] = findpeaks(spatialTunings(unit , :));
...
placeCells = find(spatialInfo > 0.5 & conslapsRatio > 0.4);


[~, sortIdx] = sort(peakPosBin(placeCells), 'ascend');
