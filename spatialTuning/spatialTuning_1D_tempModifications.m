function [spatialTunings, PF_sorted, template, spatialInfo, conslapsRatio, diffWithAvg, linearPoscenters] = spatialTuning_1D_tempModifications(spikeStruct, qclus, fileinfo, behavior, thetaPeriods, turningPeriods, speed, direction, posBinSize, runSpeedThresh, combinedUnits, timeUnit, FileBase)



nUnits = max(spikeStruct.unit);
...
spatialTunings.smoothed   = zeros(nUnits, nPosBins);
spatialTunings.unsmoothed = zeros(nUnits, nPosBins);
...
combinedflag  = zeros(nUnits, 1);

diffWithAvg   = zeros(numel(currDirLaps), nUnits);
conslapsRatio = zeros(nUnits, 1); % number of laps with consistent peak position for each unit
spatialInfo   = zeros(nUnits, 1); % spatial information of each unit
...
for ii = 1: length(activeUnits)
    ...
    spatialTunings.unsmoothed(unit, :) = unsmoothed_tunings;
    
    
    win = gausswindow(3,5);
    
    spatialTunings.smoothed(unit , :) = conv(unsmoothed_tunings, win, 'same');
    [~, peakPosBin(unit)] = max(spatialTunings.smoothed(unit , :));
    ...
    tempPlaceMap = spatialTunings.smoothed(unit , :) + 1e-4; % add a small value to remove zero firing rates
...
PF_sorted    = spatialTunings.smoothed(placeCells(sortIdx), :); % sort the place fields based on the peaks
...
y0=0;
width=250;%400;
height=100* size(PF_sorted_norm, 1)/20;
...
h = text(linearPoscenters(1)-5*difpos, 0.06*(tt)/2, 'Units', 'fontsize', 14, 'HorizontalAlignment', 'center');
set(h, 'rotation', 90)
% 
...
for ii = 1 : length(template)
    ...
    currSpiketimes = floor(spikeStruct.t(spkInd)*fileInfo.Fs);
