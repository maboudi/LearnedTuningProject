...
for currSession = 4%1: noSessions
...
PBErippleIdx_pre = ifContainRipples(primaryPBEs_pre, ripplePeriods);
secondaryPBEs_pre = primaryPBEs_pre(PBErippleIdx_pre == 1, :);


PREbinnedPBEs = struct('data', [], 'p', [], 'ts', [], 'pts', []);
[PREbinnedPBEs.data, secondaryPBEs_pre, nFiringUnits_pre] = finalBinningResult(secondaryPBEs_pre, spikeStruct, fileinfo); %%%% CHANGED SECONDARY TO PRIMARY
...
savePBEs(primaryPBEs_pre, secondaryPBEs_pre, PBErippleIdx_pre, nFiringUnits_pre, fileinfo, subfolder)
...
velocityFilter  = 1;
[primaryPBEs_run, sdat_run] = PBPeriods(spikeStruct, fileinfo, [], time_resolution, threshZ, exclude, velocityFilter);

PBErippleIdx_run = ifContainRipples(primaryPBEs_run, ripplePeriods);
secondaryPBEs_run = primaryPBEs_run(PBErippleIdx_run == 1, :);


RUNbinnedPBEs = struct('data', [], 'p', [], 'ts', [], 'pts', []);
[RUNbinnedPBEs.data, secondaryPBEs_run, nFiringUnits_run] = finalBinningResult(secondaryPBEs_run, spikeStruct, fileinfo);
...
savePBEs(primaryPBEs_run, secondaryPBEs_run, PBErippleIdx_run, nFiringUnits_run, fileinfo, subfolder)
...
PBErippleIdx_post = ifContainRipples(primaryPBEs_post, ripplePeriods);
secondaryPBEs_post = primaryPBEs_post(PBErippleIdx_post == 1, :);


POSTbinnedPBEs = struct('data', [], 'p', [], 'ts', [], 'pts', []);
[POSTbinnedPBEs.data, secondaryPBEs_post, nFiringUnits_post] = finalBinningResult(secondaryPBEs_post, spikeStruct, fileinfo);
...
savePBEs(primaryPBEs_post, secondaryPBEs_post, PBErippleIdx_post, nFiringUnits_post, fileinfo, subfolder)



% sequenceMatchingIndexAnalysis
