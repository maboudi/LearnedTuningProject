...
mySpikes = spikeBehaviorAnalysis3(spikes, speed, qual2consider, fileinfo);
...
cntPBES_entire = [cntPBES_PRE cntPBES_RUN cntPBES_POST];
...
% 
% 
% [PRE_RUN, sortPRE_RUN] = stateCorrespondence(cntPBES_entire, lambdaPRE, transmatPRE, lambdaRUN, transmatRUN, uniformTransMat, normalizeROW);
% [RUN_PRE, sortRUN_PRE] = stateCorrespondence(cntPBES_entire, lambdaRUN, transmatRUN, lambdaPRE, transmatPRE, uniformTransMat, normalizeROW);
% 
% [RUN_POST, sortRUN_POST] = stateCorrespondence(cntPBES_entire, lambdaRUN, transmatRUN, lambdaPOST, transmatPOST, uniformTransMat, normalizeROW);
% [POST_RUN, sortPOST_RUN] = stateCorrespondence(cntPBES_entire, lambdaPOST, transmatPOST, lambdaRUN, transmatRUN, uniformTransMat, normalizeROW);
% 
% [PRE_POST, sortPRE_POST] = stateCorrespondence(cntPBES_entire, lambdaPRE, transmatPRE, lambdaPOST, transmatPOST, uniformTransMat, normalizeROW);
% [POST_PRE, sortPOST_PRE] = stateCorrespondence(cntPBES_entire, lambdaPOST, transmatPOST, lambdaPRE, transmatPRE, uniformTransMat, normalizeROW);
% 

[PRE_RUN, ~, sortPRE_RUN] = stateCorrespondence2(cntPBES_entire, lambdaPRE, transmatPRE, lambdaRUN, transmatRUN, uniformTransMat, normalizeROW);
[RUN_PRE, ~, sortRUN_PRE] = stateCorrespondence2(cntPBES_entire, lambdaRUN, transmatRUN, lambdaPRE, transmatPRE, uniformTransMat, normalizeROW);

[RUN_POST, ~, sortRUN_POST] = stateCorrespondence2(cntPBES_entire, lambdaRUN, transmatRUN, lambdaPOST, transmatPOST, uniformTransMat, normalizeROW);
[POST_RUN, ~, sortPOST_RUN] = stateCorrespondence2(cntPBES_entire, lambdaPOST, transmatPOST, lambdaRUN, transmatRUN, uniformTransMat, normalizeROW);

[PRE_POST, ~, sortPRE_POST] = stateCorrespondence2(cntPBES_entire, lambdaPRE, transmatPRE, lambdaPOST, transmatPOST, uniformTransMat, normalizeROW);
[POST_PRE, ~, sortPOST_PRE] = stateCorrespondence2(cntPBES_entire, lambdaPOST, transmatPOST, lambdaPRE, transmatPRE, uniformTransMat, normalizeROW);
...
figure;
set(gcf, 'position', [ -1375 911 895 228])


% subplot(1,6,1); plotPanel(PRE_RUN, sortPRE_RUN, 'PRE', 'RUN', 'ENT')

subplot(1,3,1); plotPanel(RUN_PRE, sortRUN_PRE, 'RUN', 'PRE', 'ENT')

% subplot(1,6,3); plotPanel(RUN_POST, sortRUN_POST, 'RUN', 'POST', 'ENT')

subplot(1,3,2); plotPanel(POST_RUN, sortPOST_RUN, 'POST', 'RUN', 'ENT')

% subplot(1,6,5); plotPanel(PRE_POST, sortPRE_POST, 'PRE', 'POST', 'ENT')

subplot(1,3,3); plotPanel(POST_PRE, sortPOST_PRE, 'POST', 'PRE', 'ENT')

%%
savepdf(gcf, [ fileinfo.name '_stateCorrespondence'])
...
[POSTstates_run, RUNstates] = find(POST_RUN > -log10(0.05/30/50));
[POSTstates_pre, PREstates_AlsoinPOST] = find(POST_PRE > -log10(0.05/50/50));
[RUNstates_pre, PREstates_AlsoinRUN] = find(RUN_PRE > -log10(0.05/30/50));
...
POST_RUN_simChance = Lambdasimilarity(lambdaPOST(:, randi(50, 1, noRandomization)), lambdaRUN(:, randi(30, 1, noRandomization)));
% POST_RUN_all = Lambdasimilarity(lambdaPOST, lambdaRUN);


POST_PRE_sim = Lambdasimilarity(lambdaPOST(:, POSTstates_run), lambdaPRE(:, PREstates_AlsoinPOST));
POST_PRE_simChance = Lambdasimilarity(lambdaPOST(:, randi(50, 1, noRandomization)), lambdaPRE(:, randi(50, 1, noRandomization)));
...
RUN_PRE_simChance = Lambdasimilarity(lambdaRUN(:, randi(30, 1, noRandomization)), lambdaPRE(:, randi(50, 1, noRandomization)));
...
qclus = 1; % Only the pyramidal neurons were included
...
function [A_B_corresp_pval, A_B_corresp_norm,  sortAstates] = stateCorrespondence2(testData, lambdaA, transmatA, lambdaB, transmatB, uniformTranprobs, normalizeROW)
...
gammagivenA_chance = zeros(size(B, 1), size(B,2), noShuffles);

for ii = 1: noShuffles
    [~, ~, gammagivenA_chance(:,:, ii)] = fwdback(prior, currTransmat, B(:, randperm(size(B, 2))));
end
...
gammagivenB_chance = zeros(size(B, 1), size(B,2), noShuffles);

for ii = 1: noShuffles
    [~, ~, gammagivenB_chance(:,:, ii)] = fwdback(prior, currTransmat, B(:, randperm(size(B, 2))));
end


gammagivenA_chance2 = repmat(mean(gammagivenA, 2), [1, noTimebins]);
gammagivenB_chance2 = repmat(mean(gammagivenB, 2), [1, noTimebins]);
...
for ii = 1:noShuffles
    A_B_corresp_chance(:,:, ii) = (gammagivenA_chance(:,:, ii) * gammagivenB_chance(:,:, ii)')/noTimebins;
end
...
A_B_corresp_chance2 = (gammagivenA_chance2 * gammagivenB_chance2')/noTimebins;


A_B_corresp_norm = A_B_corresp - A_B_corresp_chance2;
...
imagesc(stateCorrespMat(rowSortInd, :), [0 150]); colormap('jet'); % , [-0.01 0.01]

set(gca, 'YDir', 'normal', 'ytick', 1:length(rowSortInd), 'yticklabel', rowSortInd, 'fontsize', 10)
set(gca, 'XTick', [], 'YTick', [])

ylabel([model ' states'] , 'fontsize', 14)
xlabel([model2comp ' states'], 'fontsize', 14)
