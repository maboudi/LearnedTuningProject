function generate_eventInfo_BapunSessions2(sessionNumber)
...
outputDir = fullfile(parentDir, 'sessions_calculated_PBEinfo4');
if ~exist(outputDir, 'dir'); mkdir(outputDir); end

sessionNames = {'RatN_Day2_2019-10-11_03-58-54'; 'RatS-Day2-2020-11-27_10-22-29'; 'RatU_Day2NSD_2021-07-24_08-16-38'};
...
mazeShapes = {'L-shape'; 'L-shape'; 'circular'};
...
varList = {'spike'; 'epochs'; 'positions'}; % ; 'iso_pos'
...
timePoints = brainState.thetaTimePnts;
...
% fileName = fullfile(fullfile(stateDetectionResultsDir, sessionName, [sessionName '.nrm.evt']));
% MakeEvtFile(brainState.periods(brainState.periods(:,3)==1, 1:2), fileName, {'beg', 'end'}, 1, 1)
...
%         fileInfo.RippleChannels = best_channels; % check if there are no bad channels among the best_channels 
        fileInfo.RippleChannels = [171 183 142 48 60 74 32 151 86 101 114 16];
    ...
    elseif sessionNumber == 3 % rat U
        
        scaleFactor = 4;
        position_samplingRate = double(sampling_rate);
        positions = traces/scaleFactor;
        
        temp = zeros(size(positions));
        shift = 53; 
        temp(:, shift+1:end)=positions(:, 1:end-shift);
        positions = temp;
        
        linearPos = iso_pos(: ,1)/scaleFactor;
        temp = zeros(size(linearPos));
        temp(shift+1:end)=linearPos(1:end-shift);
        linearPos = temp;


        % REMAZE

        shift = 108; % extra shift to what have been calculated for MAZE

        temp = zeros(size(positions));
        
        temp(:, 1:end-shift) = positions(:, shift+1:end);
        positions = temp;

        temp = zeros(size(linearPos));
        temp(1:end-shift)=linearPos(shift+1:end);
        linearPos = temp;
...
%     if sessionNumber == 3
%         
%         idx = find(fileInfo.xyt(:,3) >= behavior.time(2,1) & fileInfo.xyt(:,3) < behavior.time(2,2));
%         rr  = nan(size(fileInfo.xyt));
%         rr(idx, :) = fileInfo.xyt(idx, :);
%         linearPos = iso_pos(idx, 1);
%     end
%     fileInfo.xyt = rr;
...
%     spikes = rmfield(spikes, {'t'; 'shank'; 'q'; 'sh'; 'Var1'});
    ...
    spikes_pyr2 = spatialTuning_1D_June21(spikes_pyr, behavior, [], [], fileInfo.speed, runSpeedThresh, 'LR', posBinSize, fileInfo, subfolder);
    spikes_pyr2 = spatialTuning_1D_June21(spikes_pyr, behavior, [], [], fileInfo.speed, runSpeedThresh, 'RL', posBinSize, fileInfo, subfolder);

    spikes_pyr2 = spatialTuning_1D_June21(spikes_pyr, behavior, [], [], fileInfo.speed, runSpeedThresh, 'uni', posBinSize, fileInfo, subfolder);
    ...
    fileName = fullfile(folderName, [sessionName '.spikes.mat']);
    save(fileName, 'spikes', 'spikes_pyr2', 'fileInfo', '-v7.3') % , 'clusterQuality'
...
if exist(folderName, 'dir') && overwrite == 0
    ...
    pbeDetectParams.exclude                = brainState.periods(ismember(brainState.periods(:, 3), [2 3]), 1:2); % NREM and QW periods
