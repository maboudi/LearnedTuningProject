...
% [RUN_POST, ~, sortRUN_P+OST] = stateCorrespondence2(cntPBES_entire, lambdaRUN, transmatRUN, lambdaPOST, transmatPOST, uniformTransMat, normalizeROW);
...
% Decoded states given model A
ObsPA = poisson_prob(testData, lambdaA, 1);
...
[~,~, gammagivenA] = fwdback(prior, currTransmatA, ObsPA);



% Decoded states given model B
ObsPB = poisson_prob(testData, lambdaB, 1);
...
[~,~, gammagivenB] = fwdback(prior, currTransmatB, ObsPB);



gammagivenA_chance = zeros(size(ObsPA, 1), size(ObsPA,2), noShuffles);
gammagivenB_chance = zeros(size(ObsPB, 1), size(ObsPB,2), noShuffles);

for ii = 1: noShuffles
    shuffleTBins = randperm(size(ObsPA, 2));
    [~, ~, gammagivenA_chance(:,:, ii)] = fwdback(prior, currTransmatA, ObsPA(:, shuffleTBins));
%     shuffleTBins = randperm(size(ObsPB, 2));
    [~, ~, gammagivenB_chance(:,:, ii)] = fwdback(prior, currTransmatB, ObsPB(:, shuffleTBins));
end
...
imagesc(stateCorrespMat(rowSortInd, :)); colormap('jet'); % , [0 150]
...
% 
% 
% imagesc(stateCorrespMat(rowSortInd, :), [-0.01 0.01]); colormap('jet');
