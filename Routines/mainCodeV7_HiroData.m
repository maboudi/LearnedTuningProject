...
mazeShape = {'linear'; 'linear'; 'linear'; 'linear'; 'L-shape'; 'U-shape'; 'linear'};
...
for sessionNumber = 3%sessionNumbers
...
behavior.time(2,1) = behavior.time(2,1) + 1e6;
...
linearPos = linearizePosition(fileinfo, behavior, mazeShape{t});

fileinfo.xyt(:, 4) = linearPos; % adding the linearizes positions to the same position field in fileinfo
...
fileinfo.xyt(:, 5) = zeros(size(linearPos)); % labeling the postion samples with the calculated laps (if not part of any lap the label is zero)

for ii = 1: length(laps)

   idx =  find(fileinfo.xyt(:, 3) > laps(ii, 1) & fileinfo.xyt(:, 3) < laps(ii, 2));
   fileinfo.xyt(idx, 5) = ii;
...
% res files. I don't have access to the res files for now, so just keep
% going with the sorted spikes
...
spatialTunings2D_L2R = spatialTuning_2D(mySpikes, [1 2 3], fileinfo, behavior, 'right', 2, runSpeedThresh, fileinfo.Fs, fileBase);
spatialTunings2D_R2L = spatialTuning_2D(mySpikes, [1 2 3], fileinfo, behavior, 'left', 2, runSpeedThresh, fileinfo.Fs, fileBase);
...
[spatialTunings_L2R, PF_sorted_L2R,  activeUnits_sorted_L2R] = spatialTuning_1D(mySpikes, [1 2 3], fileinfo, behavior, 'right', 2, runSpeedThresh, fileinfo.Fs, fileBase);
[spatialTunings_R2L, PF_sorted_R2L,  activeUnits_sorted_R2L] = spatialTuning_1D(mySpikes, [1 2 3], fileinfo, behavior, 'left', 2, runSpeedThresh, fileinfo.Fs, fileBase);




%%
frame2framedisThresh  = 60;
noStepsThresh         = 2;
start2endThresh       = 60;
...
maxNoSteps     = zeros(length(eventsBinnedfiringPRE), 1);
maxDistCovered = zeros(length(eventsBinnedfiringPRE), 1);

for pbe = 1: length(eventsBinnedfiringPRE)
    
    currEvtInfo = PBETrajsegmentsPRE{pbe};
    
    if ~isempty(currEvtInfo)
        [maxNoSteps(pbe), bestTraj] = max(currEvtInfo(:, 3));
        maxDistCovered(pbe) = currEvtInfo(bestTraj, 5);
    end


end

PBEidx2plot = find(maxNoSteps > 2 & maxDistCovered > 60);

FileBase = [currDir '/' fileinfo.name  '/BD/replayDetection/PRE/'];
mkdir(FileBase)

binDur = 0.02;
plotBD2(eventsBinnedfiringPRE, peakPosBinPRE, distFromPrevBinPRE, PBETrajsegmentsPRE, PBEidx2plot, fileinfo, behavior, binDur, posBinSize, FileBase)
...
maxNoSteps     = zeros(length(eventsBinnedfiringRUN), 1);
maxDistCovered = zeros(length(eventsBinnedfiringRUN), 1);

for pbe = 1: length(eventsBinnedfiringRUN)
    
    currEvtInfo = PBETrajsegmentsRUN{pbe};
...
FileBase = [currDir '/' fileinfo.name  '/BD/replayDetection/RUN/'];
mkdir(FileBase)

binDur = 0.02;
plotBD2(eventsBinnedfiringRUN, peakPosBinRUN, distFromPrevBinRUN, PBETrajsegmentsRUN, PBEidx2plot, fileinfo, behavior, binDur, posBinSize, FileBase)
...
maxNoSteps     = zeros(length(eventsBinnedfiringPOST), 1);
maxDistCovered = zeros(length(eventsBinnedfiringPOST), 1);

for pbe = 1: length(eventsBinnedfiringPOST)
    
    currEvtInfo = PBETrajsegmentsPOST{pbe};
...
FileBase = [currDir '/' fileinfo.name  '/BD/replayDetection/POST/'];
mkdir(FileBase)

binDur = 0.02;
plotBD2(eventsBinnedfiringPOST, peakPosBinPOST, distFromPrevBinPOST, PBETrajsegmentsPOST, PBEidx2plot, fileinfo, behavior, binDur, posBinSize, FileBase)
