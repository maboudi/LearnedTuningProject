function [participation, participantCells, partPCs, partnonPCs] = unitsParticipation2(eventsBinnedfiring, activeUnits, RLtemplate, LRtemplate, fileinfo, longORshort, binDur)
...
meanFiring2 = meanFiring([leftonlyUnits rightonlyUnits bidirectionalUnits notplaceUnits]);

leftonlyUnits2 = 1:length(leftonlyUnits);
rightonlyUnits2 = leftonlyUnits2(end) + (1:length(rightonlyUnits));
bidirectionalUnits2 = rightonlyUnits2(end) + (1:length(bidirectionalUnits));
notplaceUnits2 = bidirectionalUnits2(end) + (1:length(notplaceUnits));

for unit = 1 : length(activeUnits)
    
    if ismember(unit, leftonlyUnits2)
        singleBarColor = 'r';
    elseif ismember(unit, rightonlyUnits2)
        singleBarColor = 'b';
    elseif ismember(unit, bidirectionalUnits2)
        singleBarColor = [135 31 120]/255;
    elseif ismember(unit, notplaceUnits2)
        singleBarColor = 'k';
    else
        singleBarColor = 'g'; % some other units which were not active on the track but fired during population burst events. 
    end
    
    bar(unit, meanFiring2(unit), 'parent', fHand, 'facecolor', singleBarColor, 'edgecolor','none')
end

% line([1 length(activeUnits)], [firing95prctile firing95prctile], 'color', 'k')
...
hold(fHand, 'on')
participationRatio2 = participationRatio([leftonlyUnits rightonlyUnits bidirectionalUnits notplaceUnits]);
...
for unit = 1 : length(activeUnits)
    ...
    bar(unit, participationRatio2(unit), 'parent', fHand, 'facecolor', singleBarColor, 'edgecolor','none')
end

% line([1 length(activeUnits)], [PR95prctile PR95prctile], 'color', 'k')
...
hold on
bar(bins, h1, 'facecolor', [135 31 120]/255, 'edgecolor', 'none')
% hold on
...
bar(bin, h1, 'facecolor', [135 31 120]/255, 'edgecolor', 'none')
