...
sbefore = 26;%samples before/after for spike extraction
safter = 26;%... could read from SpkGroups in xml
...
%         end
%     end
    channellist = par.SpkGrps(groupidx).Channels+1;
    ...
    % features: 3 from each channel (why is it 12 instead of 8??)
    
    features = reshape(permute(pcFeatures(tidx, :, :), [2 3 1]), 36, []);
...
%     fets = cat(1,wvpowers,wvranges,double(tspktimes'));
    ...
    features(:,2:end) = features(:,2:end) * 10;
    fets = cat(1, features, double(tspktimes'));
...
%      fprintf(outputfile,formatstring,Fet');
else
