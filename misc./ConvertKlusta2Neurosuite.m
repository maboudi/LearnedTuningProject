function ConvertKlusta2Neurosuite(shank,basepath,basename)
...
tkwik = fullfile(basepath,[basename '-sh' num2str(shank) '.kwik']); % '-sh0'
tkwx = fullfile(basepath,[basename '-sh' num2str(shank) '.kwx']);
clu = h5read(tkwik,['/channel_groups/' num2str(shank-1) '/spikes/clusters/main']); %% num2str(shank)
cluster_names = unique(clu);

totalch = h5readatt(tkwik,'/application_data/spikedetekt','nchannels'); %% 'n_channels'
...
channellist = h5readatt(tkwik,['/channel_groups/' num2str(shank-1)],'channel_order')+1;

%% Getting spiketimes
spktimes = h5read(tkwik,['/channel_groups/' num2str(shank-1) '/spikes/time_samples']);
...
for ind = 1:length(cluster_names)
    ...
    kk=h5readatt(tkwik,['/channel_groups/' num2str(shank-1) '/clusters/main/' num2str(cluster_names(ind))],'cluster_group');
...
%% Spike features
fets = h5read(tkwx,['/channel_groups/' num2str(shank-1) '/features_masks']);
