...
currSessions = [1:5 7:9 10 11 12 15:17 6];
...
for iUnit = 1:numel(selectedUnits)
    ...
    currMAZETuning   = spikes(currUnit).spatialTuning_smoothed.uni;
    currReMAZETuning = spikes_re(currUnit).spatialTuning_smoothed.uni;
    ...
    plot(0.2*currMAZETuning + endT{iSess}.post , 1:nPosBins, 'linewidth', 1.5, 'color', [0 0 0 0.6], 'DisplayName', 'MAZE PF')
    plot(0.2*currReMAZETuning + endT{iSess}.post, 1:nPosBins, 'linewidth', 1.5, 'color', [0 1 0 0.6], 'DisplayName', 'reMAZE PF')
    ...
    if iUnit == 4
        line(0.2*[0 2] + endT{iSess}.post, [yl(2)-10 yl(2)-10], 'linewidth', 1.5, 'color', [0 0 0 0.6])
        text(endT{iSess}.post, yl(2)+20, '2 Hz', 'fontsize', fontsize, 'color', 'k')
    end
    
    ylim([0 nPosBins]);
    yl = ylim;
    text(0, yl(2)+12, ['unit ' num2str(selectedUnits(iUnit))], 'fontsize', fontsize, 'color', 'k', 'HorizontalAlignment', 'left')
