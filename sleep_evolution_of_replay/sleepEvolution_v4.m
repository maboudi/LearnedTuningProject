...
% % this version plots only the mean of the variable across sleep
...
var_of_interest_mean_pooled_PRE = zeros(nBins_max_PRE, 1);
var_of_interest_sem_pooled_PRE  = zeros(nBins_max_PRE, 1);

for ibin = 1:nBins_max_PRE
    
    var_of_interest_mean_pooled_PRE(ibin) = nanmean(cell2mat(allSessions_PRE(:, ibin)));
    var_of_interest_sem_pooled_PRE(ibin)  = nanstd(cell2mat(allSessions_PRE(:, ibin)))/numel(cell2mat(allSessions_PRE(:, ibin)));
end
...
meanPRE = mean(var_of_interest_mean_pooled_PRE);

customPlot((var_of_interest_mean_pooled_PRE - meanPRE)/meanPRE *100 , var_of_interest_sem_pooled_PRE, currbinCenters/3600, cl, 0.5)
...
var_of_interest_mean_pooled_POST = zeros(nBins_max_POST, 1);
var_of_interest_sem_pooled_POST  = zeros(nBins_max_POST, 1);

for ibin = 1:nBins_max_POST
    
    var_of_interest_mean_pooled_POST(ibin) = nanmean(cell2mat(allSessions_POST(:, ibin)));
    var_of_interest_sem_pooled_POST(ibin)  = nanstd(cell2mat(allSessions_POST(:, ibin)))/numel(cell2mat(allSessions_POST(:, ibin)));
end
...
customPlot((var_of_interest_mean_pooled_POST-meanPRE)/meanPRE*100, var_of_interest_sem_pooled_POST, currbinCenters/3600, cl, 0.5)
