...
for iEpoch = 1:numel(epochNames)
    ...
    binCenters.(epochName) = binCenters.(epochName)(1:3:end);
    
    assemblyTunings_time.(epochName).data = assemblyTunings_time.(epochName).data(:, :, 1:3:end);
    assemblyTunings_time.(epochName).ui  = assemblyTunings_time.(epochName).ui(:, :, 1:3:end, :); 
end
...
for iUnit = 1:numel(selectedUnits)
    ...
    plot(0.1*currSpatialTuning + startT.run , 1:nPosBins, 'linewidth', 1.5, 'color', [0 0 0 0.6], 'DisplayName', 'track spatial tuning')
...
for iUnit = 1:numel(selectedUnits)
    ...
    for iEpoch = [1 3] % pre and post within-epoch consistency of learned tunings over time
        ...
        % data
        scatter(tt-0.1, epochsXCorr{iSess}.data(iEpoch, iEpoch, currUnit), 5, 'MarkerFaceColor', 'k', 'markerEdgeColor', 'none', 'MarkerFaceAlpha', 0.8);

        % ui
        currXcorrData = squeeze(epochsXCorr{iSess}.ui(iEpoch, iEpoch, currUnit, :));
        
        [xData, yData] = myViolin(currXcorrData);
        patch(tt+0.15+xData, yData, [0 0 0], 'EdgeColor', 'none', 'FaceAlpha', 0.4)
    ...
    % data
    h1 = scatter(tt-0.1, epochsXCorr{iSess}.data(1, 3, currUnit), 5, 'MarkerFaceColor', 'k', 'markerEdgeColor', 'none', 'MarkerFaceAlpha', 0.8);

    % ui
    currXcorrData = squeeze(epochsXCorr{iSess}.ui(1, 3, currUnit, :));

    [xData, yData] = myViolin(currXcorrData);    
    h2 = patch(tt+0.15+xData, yData, [0 0 0], 'EdgeColor', 'none', 'FaceAlpha', 0.4);
...
for iEpoch = [1 3]
    ...
    currData = squeeze(epochsXCorr_zscore{iSess}(iEpoch, iEpoch, okUnits, 1));

    scatter(tt+0.05*randn(numel(okUnits), 1), currData, 1, 'MarkerFaceColor', 'k', 'markerEdgeColor', 'none', 'MarkerFaceAlpha', 0.5);

    [xData, yData] = myViolin(currData);
...
% currData = squeeze(epochsXCorr{iSess}.data(1, 3, okUnits, 1));
% 
% scatter(tt+0.05*randn(numel(okUnits), 1), currData, 1, 'MarkerFaceColor', 'k', 'markerEdgeColor', 'none', 'MarkerFaceAlpha', 0.5);
% 
% [xData, yData] = myViolin(currData);
% patch(tt+xData, yData, [0 0 0], 'EdgeColor', 'none', 'FaceAlpha', 0.4)
% 
% % scatter(idx+0.4, nanmedian(currData), 5, 'MarkerFaceColor', 'w', 'MarkerEdgeColor', 'k') 
